...
@pytest.mark.skip(reason='Cookiecutter does not play nice with us, '
                         'i.e. is not able to update, since it raises an '
                         'error if the directory already exists.')
@pytest.mark.usefixtures('cookiecutter_config')
def test_namespace_cookiecutter_update(cwd):
    # Given a project was generated by putup
    # with --namespace and --cookiecutter
    run('putup myproj --namespace nested.ns --cookiecutter ' + COOKIECUTTER)
    # when we update it
    run('putup myproj --update --travis')
    # then the module hierarchy should be kept
    assert isdir('myproj/src/nested/ns/myproj')
    assert not exists('myproj/src/myproj')
    # and lastly included options should be applied
    assert exists('myproj/.travis.yml')
